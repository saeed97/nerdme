...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if not hasattr(settings, "groups_MAIL_TRACKERS"):
            logger.error("missing groups_MAIL_TRACKERS setting")
            sys.exit(1)

        worker_name = options["worker_name"]
        tracker = settings.groups_MAIL_TRACKERS.get(worker_name, None)
        if tracker is None:
            logger.error("couldn't find configuration for %r in groups_MAIL_TRACKERS", worker_name)
            sys.exit(1)
