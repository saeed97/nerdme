...
def test_view_reorder(groups_setup, admin_client):
    url = reverse("groups:reorder_tasks")
    response = admin_client.get(url)
...
def test_view_task_detail(groups_setup, admin_client):
    task = Task.objects.first()
    url = reverse("groups:task_detail", kwargs={"task_id": task.id})
    response = admin_client.get(url)
    assert response.status_code == 200


def test_del_task(groups_setup, admin_user, client):
    task = Task.objects.first()
    url = reverse("groups:delete_task", kwargs={"task_id": task.id})
...
def test_task_toggle_done(groups_setup, admin_user, client):
    task = Task.objects.first()
    assert not task.completed
    url = reverse("groups:task_toggle_done", kwargs={"task_id": task.id})
    # View accepts POST, not GET
    client.login(username="admin", password="password")
    response = client.get(url)
    assert response.status_code == 403
...
@pytest.mark.django_db
def test_no_javascript_in_task_note(groups_setup, client):
    ...
    url = reverse("groups:list_detail", kwargs={"list_id": task_list.id, "list_slug": task_list.slug})
...
def test_created_by_unchanged(groups_setup, client):
    ...
    url_add_task = reverse(
        "groups:list_detail", kwargs={"list_id": task_list.id, "list_slug": task_list.slug}
    )
    ...
    url_edit_task = reverse("groups:task_detail", kwargs={"task_id": task.id})
...
@pytest.mark.django_db
def test_no_javascript_in_comments(groups_setup, client):
    user = get_user_model().objects.get(username="u2")
    client.login(username="u2", password="password")
...
def test_view_task_mine(groups_setup, client):
...
def test_view_task_my_group(groups_setup, client, django_user_model):
...
def test_view_task_not_in_my_group(groups_setup, client):
