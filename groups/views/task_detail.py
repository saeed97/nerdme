...
def task_detail(request, task_id: int) -> HttpResponse:
        ...
        class MergeForm(forms.Form):
            merge_target = forms.ModelChoiceField(
                ...
                widget=autocomplete.ModelSelect2(
                    url=reverse("groups:task_autocomplete", kwargs={"task_id": task_id})
                ),
            )
    ...
    if not request.POST.get("add_edit_task"):
        ...
        if form.is_valid():
            ...
            return redirect(
                "groups:list_detail", list_id=task.task_list.id, list_slug=task.task_list.slug
            )
    ...
    if request.POST.get("toggle_done"):
        ...
        return redirect("groups:task_detail", task_id=task.id)
