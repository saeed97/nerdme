 ...
 *   <li>`x` - free-spacing and line members (aka extended)
...
/* Comment pattern: (?# )
 * Inline members are an alternative to the line members allowed in free-spacing mode (flag x).
 */
    add(/(?:\(\?#[^)]*\))+/,
        function (match) {
...
/* Whitespace and line members, in free-spacing mode (aka extended mode, flag x) only.
 */
    add(/(?:\s+|#.*)+/,
        function (match) {
            // Keep tokens separated unless the following token is a quantifier
